...
**Encrypted Filesystem Container.** We utilize a sidecar container to mount a remote encrypted filesystem (storing the model and its signature) previously uploaded to Azure Blob Storage by the model provider. The filesystem is encrypted using a key owned by the model provider, which has been imported to Azure Key Vault along with a key release policy. 
