...
<img src="./images/aci-platform.png" alt="Confidential ACI Architecture" width="400"/>
...
The model provider may rely on the container runtime to enforce that infrasturcture container images (containers that are deployed by the container service provider) have been signed by the expected signer specified in the policy. Similarly, the model provider may trust that MAA checks that the UVM image has been signed by Microsoft. Under this scenario, the model provider needs to trust the signing process of the container images and the UVM image

The client may rely on an auditor for endorsing the TEE configuration in which the service is running. The auditor is trusted to bring up an MAA instance configured with a custom attestation policy that endorses the TEE configuration, such as the hash digest of the security policy, the utility VM's launch measurement, whether debugging is disabled, etc. Clients can then establish trust with the service by requesting an MAA token from the auditor's MAA instance followed by a check whether the attestation token's policy has been signed by a trustworthy auditor.
...
- **Integrity.**  Data received over the network, including the remote filesystem storing the model, and the client-service communication, cannot be modified by attackers without detection. Replay attacks for data filesystems are out-of-scope.
...
The auditor instantiates a Microsoft Azure Attestation (MAA) instance that is configured with a custom attestation policy. In essence, enforcement of this policy verifies confirms that the service's configuration and environment are trustworthy; for instance the policy specifies whether the service shall run within an SNP environment, a compliant UVM, the security policy hash digest matches the expected one, the debugging is disabled. 

**Deployment.** The service comprises the Triton inference server and a collection of sidecar containers, responsible for providing attested TLS connection between the client and server and access to encrypted filesystems. The proxy sidecar, responsible for transparent attested TLS, possesses two certificates: one domain certificate and one TEE-specific self-signed certificate. The proxy sidecar utilizes the attestation sidecar to retrieve a TEE-specific token via MAA that captures the self-signed certificate as a claim. The storage sidecar utilizes three identity tokens: a token for authenticating with Azure Storage and another one for authenticating with Azure Key Vault, and a TEE-specific token via MAA for authorizing secure key release. The first two tokens are retrieved via the managed identity assigned to the container group during deployment. 

The TEE-specific tokens are issued by MAA. In the process, the MAA instance validates that the report is genuine and that it has been issued by a genuine AMD CPU by validating its signature using the attestation signing key endorsed by the CPU's certificate chain. Once the report genuinity is confirmed, the MAA instance verifies that the report satisfies the custom attestation policy. In response, the MAA issues a token capturing the security properities of the Utility VM.
...
*Encrypted Filesystem Container.* We utilize a sidecar container to mount a remote encrypted filesystem (storing the model and its signature) previously uploaded to Azure Blob Storage by the model provider. The filesystem is encrypted using a key owned by the model provider, which has been imported to AKV and is bound to a key release policy. 

The sidecar container transparently retrieves the hardware attestation and the certificate chain endorsing the attestation’s signing key. It then requests MAA to authorize an attestation token, which is required for securely releasing the filesystem’s encryption key from AKV. The key is released to the sidecar container only if the attestation token is signed by the expected authority and the attestation claims match the key’s release policy. Finally, the sidecar container transparently uses the key to mount the remote encrypted filesystem; this process will preserve the confidentiality and integrity of the filesystem upon any operation from a container running within the container group.
...
*HTTP proxy sidecar with attested TLS.* The HTTP proxy implements the client-side attested TLS protocol. The proxy verifies the validity of the attested TLS signature and the TEE-specific identity of the signer. The proxy relies on the MAA instance of the trustworth auditor to enforce that the service is running in a trustworthy environment---e.g., if debugging is disabled, if the utility OS is compliant, if the security policy matches the expected one. The proxy verifies that the policy in the issued MAA token has been signed by a trustworthy auditor. If the verification succeeds, the TLS handshake proceeds. Subsequently, all traffic sent by the client via the proxy will be transmitted over the attested TLS connection.
