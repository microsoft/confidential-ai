# Confidential ML Inference Architecture
...
ACI customers are expected to import their data/application encryption keys (for decrypting their sensitive data) to a key management service that supports secure key release, such as the Azure Key Vault, which enables users to import keys along with a secure key release policy; an example of such policy specifies the expected security policy and utility OS.  The keys can then be released to the container group if the container group presents hardware-backed attestation claims that match the key release policy.  

## Confidential ML Inference

![Confidential ML Inference Architecture](./images/conf-ml-architecture.png)

The figure illustrates the proposed confidential ML inference service. The model provider uploads their model encryption key and encrypted model to Azure services. The client uses an http proxy that establishes an attested TLS connection with the service, over which they submit inference requests. The service comprises the Triton inference server and a collection of sidecar containers, responsible for providing attested TLS connection between the client and server, secure key release, and access to encrypted filesystems. The proxy sidecar, responsible for transparent attested TLS, possesses two certificates: one domain certificate and one TEE-specific self-signed certificate. The storage sidecar utilizes three identity tokens: a token for authenticating with Azure Storage and another one for authenticating with AKV, and a TEE-specific token (via Microsoft Azure Attestation) for authorizing secure key release. The first two tokens are retrieved via managed identity assigned to the container group hosting the service. The managed identity is assigned the 

### Threat Model
We assume a powerful attacker who may gain control of the Azure infrastructure  as well as the communication channel between the client and the service. This means that all code running outside the container group’s TEE (i.e., the utility SEV-SNP VM) is untrusted. The trusted computing base (TCB) excludes the host operating system and the hypervisor. The TCB includes the utility VM, including the guest operating system and the user-space root filesystem, including the container runtime.

The confidential ML service  relies on a collection of sidecar containers to provide secure communication between client and server and to mount encrypted filesystems storing confidential models. These sidecar containers run within the container group, and hence are included in the attestation and are trusted.  Finally, these sidecars rely on Microsoft Azure Attestation and Azure Key Vault for retrieving the model encryption keys. These services are part of the TCB, run in remote TEEs, and hence they are attestable and protected from Azure infrastructure and operators.
...
**Encrypted Filesystem Container.** We utilize a sidecar container to mount a remote encrypted filesystem (storing the model and its signature) previously uploaded to Azure Blob Storage by the model provider. The filesystem is encrypted using a key owned by the model provider, which has been imported to the Azure Key Vault managed HSM service along with a key release policy. 
