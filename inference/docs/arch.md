...
The client may rely on an auditor for endorsing the security policy enforced by the container runtime. The auditor is trusted to bring up an MAA instance configured with a custom attestation policy that endorses the hash digest of the security policy and the utility VM's launch measurement. Clients can then establish trust with the service by requesting an MAA token from the auditor's MAA instance and checking whether the token's policy has been signed by a trustworthy auditor.
